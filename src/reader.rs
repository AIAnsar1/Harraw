...
pub fn hrw_read_file(filepath: &str) -> String {
...
pub fn hrw_read_file_as_yml(filepath: &str) -> Vec<yaml_rust::Yaml> {
    let content = hrw_read_file(filepath);
    yaml_rust::YamlLoader::load_from_str(content.as_str()).unwrap()
}

pub fn hrw_read_yaml_doc_accessor<'a>(doc: &'a yaml_rust::Yaml, accessor: Option<&str>) -> &'a Vec<yaml_rust::Yaml> {
...
pub fn hrw_read_file_as_yml_array(filepath: &str) -> yaml_rust::yaml::Array {
    let path = Path::new(filepath);
    let display = path.display();
...
pub fn hrw_read_csv_file_as_yml(filepath: &str, quote: u8) -> yaml_rust::yaml::Array {
    let path = Path::new(filepath);
    let display = path.display();
    let file = match File::open(path) {
        Err(why) => panic!("couldn't open {}: {}", display, why),
